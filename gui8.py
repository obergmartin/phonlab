...
class WaveformCanvas(ThrottledCanvas):
    ...
    def plot_waveform(self, data):
        ...
        self.wave_x = np.arange(data.shape[0])
        self.wave_y = data
        self.axes.clear()
        self.axes.plot(self.wave_x, self.wave_y, linewidth=0.8, antialiased=False)
        # fix y-limits to avoid autoscale cost
        self.axes.set_ylim(np.min(self.wave_y), np.max(self.wave_y))
        # full redraw now
        self.draw()
